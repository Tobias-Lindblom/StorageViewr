...
export default async function InventoriesPage({
  ...
  return (
    <AppShell context={context}>
      <PageHeading
        title="Inventering"
        description="Räkna på plats. Granska avvikelser. Uppdatera lagret."
        action={
          admin ? (
            ...
            >
              ...
              <div className="mt-6 flex flex-wrap justify-between gap-2 text-xs">
                <span className="text-cyan">
                  {item.status === "completed" ? "Genomförd" : "Pågående"}
                </span>
                <span>
                  {item.counted} av {item.total} platser
                </span>
              </div>
              <progress
