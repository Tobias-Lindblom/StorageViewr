...
} from "@/validation/inventory-session";

function conflict(
  message = "Saldot eller räkningen har ändrats. Läs in platsen igen och bekräfta din räkning.",
): never {
...
async function lockWarehouse(
  ...
  const warehouse = await Warehouse.findOneAndUpdate(
    { _id: id, organizationId: context.organizationId, active: true },
...
async function lockInventory(
  ...
  return (await InventorySession.findOneAndUpdate(
    filter,
    { $inc: { revision: 1 } },
...
async function currentRows(
  ...
  const products = await Product.find({
    organizationId: context.organizationId,
    _id: { $in: levels.map((row) => row.productId) },
    active: true,
  })
    .session(session)
    .lean();
  ...
  return db.connection.transaction(async (session) => {
    ...
    if (locations.length !== data.locationIds.length)
      throw new AppError(
        400,
        "INVALID_SCOPE",
        "Alla valda platser måste vara aktiva och tillhöra det valda lagret.",
      );
    ...
    const levels = await currentRows(
      context,
      locations.map((row) => row._id),
      session,
    );
    ...
    const starter = await User.findById(context.userId)
      .select("name")
      .session(session)
      .lean();
    const [inventory] = await InventorySession.create(
      [
        {
          organizationId: context.organizationId,
          warehouseId: warehouse._id,
          ...
          startedByName: starter?.name ?? "Ej sparat",
        },
      ],
      { session },
    );
    await InventorySessionLocation.insertMany(
...
export async function saveLocationCount(
  ...
  return db.connection.transaction(async (session) => {
    ...
    if (inventory.status !== "active")
      conflict("Inventeringen är genomförd och kan inte ändras.");
    const scopeFilter = {
      organizationId: context.organizationId,
      inventorySessionId: inventory._id,
      locationId,
    ...
    const scope =
      await InventorySessionLocation.findOne(scopeFilter).session(session);
    if (!scope)
      throw new AppError(
        404,
        "LOCATION_NOT_IN_SCOPE",
        "Platsen ingår inte i inventeringen.",
      );
    if (scope.revision !== data.expectedRevision) conflict();
    if (scope.status === "counted" && !data.confirmReplace)
      conflict("Bekräfta att den tidigare räkningen ska ersättas.");
    ...
    const products = await Product.find({
      organizationId: context.organizationId,
      _id: { $in: data.rows.map((row) => row.productId) },
      active: true,
    })
      .session(session)
      .lean();
    if (products.length !== data.rows.length)
      throw new AppError(
        400,
        "INVALID_PRODUCT",
        "Välj aktiva produkter i ditt företag.",
      );
    await Product.updateMany(
      {
        organizationId: context.organizationId,
        _id: { $in: products.map((row) => row._id) },
      },
      { $inc: { stockRevision: 1 } },
      { session },
    );
    ...
    if (levels.some((row) => !submitted.has(String(row.productId))))
      conflict(
        "Platsens produkter har ändrats eller saknas i räkningen. Läs in platsen igen.",
      );
    ...
    const counts = data.rows.map((row) => {
      ...
      return {
        ...
        sku: product.sku,
        expectedQuantity: level?.quantity ?? 0,
        expectedVersion: level?.version ?? null,
        countedQuantity: row.countedQuantity,
        difference: row.countedQuantity - (level?.quantity ?? 0),
        countedBy: context.userId,
        countedByName: actor?.name ?? "Användare",
        countedAt: new Date(),
      };
    });
    ...
    await InventorySessionLocation.updateOne(
      scopeFilter,
      {
        $set: {
          status: "counted",
          completedBy: context.userId,
          completedByName: actor?.name ?? "Användare",
...
export async function finishInventory(
  ...
  return db.connection.transaction(async (session) => {
    ...
    if (inventory.revision !== data.expectedRevision)
      conflict(
        "Räkningarna har ändrats sedan granskningen. Läs in inventeringen igen.",
      );
    const filter = {
      organizationId: context.organizationId,
      inventorySessionId: inventory._id,
    };
    const scopes = await InventorySessionLocation.find(filter)
      .session(session)
      .lean();
    if (!scopes.length || scopes.some((row) => row.status !== "counted"))
      conflict("Alla lagerplatser måste räknas och bekräftas före avslut.");
    const counts = await InventoryCount.find(filter).session(session).lean();
    const locations = await Location.find({
      organizationId: context.organizationId,
      warehouseId: inventory.warehouseId,
      _id: { $in: scopes.map((row) => row.locationId) },
      active: true,
    })
      .session(session)
      .lean();
    if (locations.length !== scopes.length)
      conflict("En lagerplats är inte längre aktiv.");
    const levels = await currentRows(
      context,
      locations.map((row) => row._id),
      session,
    );
    const key = (product: unknown, location: unknown) =>
      String(product) + ":" + String(location);
    const countMap = new Map(
      counts.map((row) => [key(row.productId, row.locationId), row]),
    );
    if (levels.some((row) => !countMap.has(key(row.productId, row.locationId))))
      conflict(
        "Nya produkter har tillkommit på en räknad plats. Räkna platsen igen.",
      );
    const levelMap = new Map(
      levels.map((row) => [key(row.productId, row.locationId), row]),
    );
    // Validate every version before writing a single adjustment.
    for (const count of counts) {
      if (!locations.some((location) => location._id.equals(count.locationId)))
        conflict();
      if (
        (levelMap.get(key(count.productId, count.locationId))?.version ??
          null) !== count.expectedVersion
      )
        conflict(
          "Saldon har ändrats sedan räkningen. Räkna om de markerade platserna före avslut.",
        );
    }
    const products = await Product.find({
      organizationId: context.organizationId,
      _id: { $in: counts.map((row) => row.productId) },
      active: true,
    })
      .session(session)
      .lean();
    const productMap = new Map(products.map((row) => [String(row._id), row]));
    if (counts.some((row) => !productMap.has(String(row.productId))))
      conflict("En räknad produkt är inte längre aktiv.");
    await Location.updateMany(
      {
        organizationId: context.organizationId,
        _id: { $in: locations.map((row) => row._id) },
      },
      { $inc: { stockRevision: 1 } },
      { session },
    );
    ...
    for (const count of counts) {
      ...
      if (level) {
        if (level.version >= Number.MAX_SAFE_INTEGER)
          conflict("Saldots versionsgräns har uppnåtts.");
        const updated = await InventoryLevel.updateOne(
          { ...pair, version: count.expectedVersion },
          { $set: { quantity: count.countedQuantity }, $inc: { version: 1 } },
        ...
        [
          {
            ...
            inventorySessionId: inventory._id,
            type: "INVENTORY",
            previousQuantity: count.expectedQuantity,
            ...
            sku: count.sku,
            locationCode: scopes.find((row) =>
              row.locationId.equals(count.locationId),
            )!.locationCode,
            warehouseName: inventory.warehouseName,
            reason: "Inventering: " + inventory.name,
          },
        ],
        { session },
      );
    ...
    await InventorySession.updateOne(
      { _id: inventory._id, organizationId: context.organizationId },
      {
        $set: {
          status: "completed",
          completedAt: new Date(),
          completedBy: context.userId,
          completedByName: actor?.name ?? "Ej sparat",
