  ...
  return {
    ...
    items: records.map((row) => {
      const places = scopes.filter((scope) =>
        scope.inventorySessionId.equals(row._id),
      );
      return {
        id: String(row._id),
        name: row.name,
...
export async function getInventory(context: TenantContext, id: string) {
  ...
  return db.connection.transaction(async (session) => {
    ...
    const levels = await InventoryLevel.find({
      ...
      locationId: { $in: scopes.map((row) => row.locationId) },
    })
      .session(session)
      .lean();
    const products = await Product.find({
      ...
      _id: {
        $in: [
          ...levels.map((row) => row.productId),
          ...counts.map((row) => row.productId),
        ],
      },
    })
      .session(session)
    ...
    const places = scopes
      .map((scope) => {
        const saved = counts.filter((row) =>
          row.locationId.equals(scope.locationId),
        );
        const current = levels.filter(
          (row) =>
            row.locationId.equals(scope.locationId) &&
            productMap.get(String(row.productId))?.active,
        );
        const ids =
          inventory.status === "completed"
            ? saved.map((row) => String(row.productId))
            : [
                ...new Set([
                  ...saved.map((row) => String(row.productId)),
                  ...current.map((row) => String(row.productId)),
                ]),
              ];
        const rows = ids
          .map((productId) => {
            const count = saved.find(
              (row) => String(row.productId) === productId,
            );
            const level = current.find(
              (row) => String(row.productId) === productId,
            );
            const product = productMap.get(productId);
            return {
              productId,
              productName: count?.productName ?? product?.name ?? "Produkt",
              sku: count?.sku ?? product?.sku ?? "",
              expectedQuantity: count?.expectedQuantity ?? level?.quantity ?? 0,
              expectedVersion: count?.expectedVersion ?? null,
              currentQuantity: level?.quantity ?? 0,
              currentVersion: level?.version ?? null,
              countedQuantity: count?.countedQuantity ?? null,
              difference: count?.difference ?? null,
              countedByName: count?.countedByName ?? null,
              countedAt: count?.countedAt.toISOString() ?? null,
              stale:
                inventory.status === "active" &&
                scope.status === "counted" &&
                (!count ||
                  (level?.version ?? null) !== count.expectedVersion ||
                  !product?.active),
            };
          })
          .sort((a, b) => a.productName.localeCompare(b.productName, "sv"));
        return {
          id: String(scope.locationId),
          code: scope.locationCode,
          status: scope.status,
          revision: scope.revision,
          completedByName: scope.completedByName ?? null,
          completedAt: scope.completedAt?.toISOString() ?? null,
    ...
    return {
      ...
      counted: places.filter((row) => row.status === "counted").length,
      places,
      discrepancies: places.flatMap((place) =>
        place.rows
