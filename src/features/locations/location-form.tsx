    ...
    <form onSubmit={submit} className="panel max-w-2xl space-y-6">
      {initial ? (
        <p className="text-sm text-muted">
          Lager:{" "}
          <strong className="text-foreground">{initial.warehouseName}</strong>
        </p>
      ) : (
        <label>
          Lager
      ...
      >
        ...
        <p className="wrap-break-word font-mono text-sm text-accent">
          {first ? (
            first === last ? (
              first
            ) : (
              <>
                {first} <span className="sr-only">till</span>
                <MaterialArrow name="forward" size={18} /> {last}
              </>
            )
          ) : (
            "Ange giltig zon, sektion och position."
          )}
        </p>
      </div>
      {initial && (
