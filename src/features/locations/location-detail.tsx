  ...
  return (
    <>
      <PageHeading
        ...
        description={
          location.active
            ? "En fysisk plats i ditt lager."
            : "Den här lagerplatsen är inaktiv."
        }
        action={
        ...
        <section className="panel">
          ...
          <dl className="grid grid-cols-2 gap-6 text-sm">
            {[
              ...
              ["Status", location.active ? "Aktiv" : "Inaktiv"],
            ].map(([label, value]) => (
              <div key={label}>
          ...
          <Link
            className="mt-7 inline-flex min-h-13 items-center gap-2 text-sm text-cyan"
            href={"/warehouses/" + location.warehouseId}
          >
            Visa {location.warehouseName} <MaterialArrow name="forward" />
          </Link>
        </section>
        {qr && (
          <section className="panel flex flex-col items-center p-5!">
            <h2 className="text-lg">Platsens QR-kod</h2>
            <Image
              src={qr.image}
            ...
            >
              Öppna QR-länken <MaterialArrow name="outward" />
            </Link>
            <Link
              className="button-secondary mt-2 w-full"
              href={"/locations/labels?warehouseId=" + location.warehouseId}
            >
              Visa etiketter
            </Link>
          </section>
