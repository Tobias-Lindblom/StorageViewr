  ...
  return <>
    ...
    <div className="grid gap-5 md:grid-cols-[1fr_280px]">
      <section className="panel"><h2>Platsuppgifter</h2><dl className="grid grid-cols-2 gap-6 text-sm">
        {[["Zon", location.zone], ["Hylla", location.shelf], ["Position", location.position], ["Status", location.active ? "Aktiv" : "Inaktiv"]].map(([label, value]) => <div key={label}><dt className="mb-2 text-muted">{label}</dt><dd className="font-medium">{value}</dd></div>)}
      </dl><Link className="mt-7 inline-flex min-h-13 items-center text-sm text-cyan" href={"/warehouses/" + location.warehouseId}>Visa {location.warehouseName} →</Link></section>
      {qr && <section className="panel flex flex-col items-center !p-5"><h2 className="text-lg">Platsens QR-kod</h2><Image src={qr.image} alt={"QR-kod för " + location.code} width={216} height={216} unoptimized className="h-auto max-w-full rounded-xl" /><Link href={qr.url} className="mt-3 inline-flex min-h-13 items-center text-sm text-cyan">Öppna QR-länken ↗</Link><Link className="button-secondary mt-2 w-full" href={"/locations/labels?warehouseId=" + location.warehouseId}>Visa etiketter</Link></section>}
    </div>
  </>;
}
