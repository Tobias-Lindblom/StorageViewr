...
export function qrOrigin() {
  ...
  if (!configured)
    throw new AppError(503, "NOT_CONFIGURED", "Applikationens adress saknas.");
  ...
  if (
    ...
    throw new AppError(
      503,
      "NOT_CONFIGURED",
      "Applikationens adress är ogiltig.",
    );
  return url.origin;
}
...
export async function getLabels(context: TenantContext, warehouseId?: string) {
  ...
  for (const location of locations) {
    labels.push({
      ...
      code: location.code,
      warehouseName: names.get(location.warehouseId.toString())!,
