  ...
  return [...warehouses.values()]
    ...
    .map((warehouse) => ({
      ...
      shelves: [...warehouse.shelves.values()]
        ...
        .map((shelf) => ({
          ...shelf,
          locations: shelf.locations.sort((a, b) =>
...
export function LocationList({
  ...
  const filtered = locations.filter((location) =>
    (location.code + " " + location.warehouseName)
      .toLocaleLowerCase("sv")
      .includes(normalized),
  );
  const groups = groupLocations(filtered);
  const shelfCount = groups.reduce(
  ...
  return (
    <div>
      <div
        className={
          warehouses
            ? "mb-5 grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]"
            : "mb-5 max-w-lg"
        }
      >
        {warehouses && (
          <div className="min-w-0">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                startTransition(() =>
                  router.push("/locations" + (id ? "?warehouseId=" + id : ""), {
                    scroll: false,
                  }),
                );
              }}
      ...
      <div
        aria-busy={pending}
        className={pending ? "space-y-8 opacity-60" : "space-y-8"}
      >
                  ...
                  <summary className="flex min-h-20 cursor-pointer list-none items-center gap-3 rounded-2xl px-4 py-4 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan sm:px-5 [&::-webkit-details-marker]:hidden">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-muted">
                        Zon {shelf.zone} · Sektion {shelf.shelf}
                      </p>
                    </div>
                    <span className="shrink-0 text-xs text-muted">
                      {shelf.locations.length}{" "}
                      {shelf.locations.length === 1 ? "plats" : "platser"}
                    </span>
                    <MaterialArrow
                      name="expand"
        ...
        <div className="border-t border-line py-8">
          ...
          <p className="max-w-md text-sm leading-7 text-muted">
            {locations.length
              ...
              : admin
                ? "Skapa platser så samlas de här efter lager, zon och sektion."
                : "En administratör behöver lägga till lagerplatser innan de visas här."}
          </p>
          {query && (
            <button
              type="button"
              className="mt-3 min-h-13 text-sm text-cyan"
              onClick={() => setQuery("")}
            >
              Rensa sökning
            </button>
