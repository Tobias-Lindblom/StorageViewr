...
function serialize(record: WarehouseRecord & { _id: Types.ObjectId }) {
  return { id: record._id.toString(), name: record.name, code: record.code,
    address: record.address, active: record.active, updatedAt: record.updatedAt.toISOString() };
}
export async function listWarehouses(context: TenantContext) {
  const filter = { organizationId: context.organizationId, ...(context.role !== "admin" ? { active: true } : {}) };
  ...
  const byWarehouse = new Map(counts.map(item => [item._id.toString(), item.count]));
  return warehouses.map(record => ({ ...serialize(record), locationCount: byWarehouse.get(record._id.toString()) ?? 0 }));
}
export async function getWarehouse(context: TenantContext, id: string) {
  ...
  if (!record) throw new AppError(404, "WAREHOUSE_NOT_FOUND", "Lagret kunde inte hittas.");
  return serialize(record);
}
export async function createWarehouse(context: TenantContext, input: unknown) {
  requireAdmin(context);
  const data = warehouseSchema.parse(input);
  return serialize(await Warehouse.create({ ...data, organizationId: context.organizationId }));
}
export async function updateWarehouse(context: TenantContext, id: string, input: unknown) {
  requireAdmin(context);
  ...
  return db.connection.transaction(async session => {
    ...
    if (!record) throw new AppError(404, "WAREHOUSE_NOT_FOUND", "Lagret kunde inte hittas.");
    if (!data.active && await Location.exists({ organizationId: context.organizationId, warehouseId, active: true }).session(session)) {
      throw new AppError(409, "WAREHOUSE_HAS_ACTIVE_LOCATIONS", "Inaktivera lagrets aktiva platser innan du inaktiverar lagret.");
    }
    return serialize(record);
  });
}
