...
import {
  ...
  listWarehouses,
  updateWarehouse,
} from "../src/features/warehouses/service";
import {
  createLocation,
  createLocationBatch,
  getLocation,
  ...
  listLocations,
  updateLocation,
} from "../src/features/locations/service";
...
test("warehouses scope reads, writes, uniqueness and privileges to their organization", async () => {
  ...
  assert.deepEqual(
    (await listWarehouses(a)).map((row) => row.id),
    [own.id],
  );
  await assert.rejects(getWarehouse(a, foreign.id), status(404));
  await assert.rejects(
    updateWarehouse(a, foreign.id, {
      ...warehouseInput,
      name: "Intrång",
      active: true,
    }),
  ...
  await assert.rejects(
    createWarehouse(a, { ...warehouseInput, code: " boras " }),
  );
  await assert.rejects(
    createWarehouse(a, {
      ...warehouseInput,
  ...
  await assert.rejects(
    createWarehouse(
      { ...a, role: "warehouse" },
      { ...warehouseInput, code: "NEW" },
    ),
    status(403),
  );
  await assert.rejects(
    updateWarehouse({ ...a, role: "warehouse" }, own.id, {
      ...warehouseInput,
      active: false,
    }),
    status(403),
...
test("location references, QR lookup, list and edit cannot cross tenant boundaries", async () => {
  const a = tenant(),
    b = tenant();
  const own = await createWarehouse(a, warehouseInput),
    foreign = await createWarehouse(b, warehouseInput);
  ...
  await assert.rejects(
    createLocation(a, { ...parts, warehouseId: foreign.id }),
    status(404),
  );
  await assert.rejects(
    createLocationBatch(a, { ...batchInput, warehouseId: foreign.id }),
    status(404),
  );
  await assert.rejects(getLocation(a, other.id), status(404));
  await assert.rejects(getLocationByToken(a, other.qrToken), status(404));
  await assert.rejects(
    updateLocation(a, other.id, { ...parts, active: false }),
    status(404),
  );
  await assert.rejects(
    updateLocation(a, location.id, {
      ...parts,
      active: true,
  ...
  assert.equal(
    labels[0].url,
    "http://localhost:3000/location/" + location.qrToken,
  );
...
test("batch creation is atomic when a normalized code already exists", async () => {
  ...
  await assert.rejects(
    createLocationBatch(a, { ...batchInput, warehouseId: warehouse.id }),
  );
  const remaining = await listLocations(a);
  assert.equal(remaining.length, 1);
  assert.equal(remaining[0].code, "A-01-05");
  const created = await createLocationBatch(a, {
    ...batchInput,
    warehouseId: warehouse.id,
    zone: "B",
  });
...
test("roles, stable QR tokens and inactive warehouse rules are enforced", async () => {
  ...
  const warehouse = await createWarehouse(a, warehouseInput);
  const location = await createLocation(a, {
    warehouseId: warehouse.id,
    ...parts,
  });
  await assert.rejects(
    createLocation(worker, {
      warehouseId: warehouse.id,
      ...parts,
      position: "02",
    }),
  ...
  await assert.rejects(
    createLocationBatch(worker, { warehouseId: warehouse.id, ...batchInput }),
    status(403),
  );
  await assert.rejects(
    updateLocation(worker, location.id, { ...parts, active: false }),
    status(403),
  );
  await assert.rejects(
  ...
  const edited = await updateLocation(a, location.id, {
    ...parts,
    position: "02",
    active: true,
  });
  ...
  assert.equal(
    (await getLocationByToken(worker, location.qrToken)).code,
    "A-01-02",
  );
  await updateLocation(a, location.id, {
    ...parts,
    position: "02",
    active: false,
  });
  ...
  await assert.rejects(
    createLocation(a, { warehouseId: warehouse.id, ...parts }),
    status(404),
  );
...
test("dashboard numbers reflect only real active data belonging to the current tenant", async () => {
  const a = tenant(),
    b = tenant();
  assert.deepEqual(await getDashboard(a), {
    warehouseCount: 0,
    locationCount: 0,
    productCount: 0,
  ...
  const foreign = await createWarehouse(b, warehouseInput);
  await createLocationBatch(b, {
    ...batchInput,
    warehouseId: foreign.id,
    count: 20,
  });
  await Product.create([
    { organizationId: a.organizationId, sku: "ACTIVE", name: "Aktiv produkt" },
...
test("validation blocks malformed references, unsafe batches and open redirects", async () => {
  ...
  for (const count of [0, -1, 101, NaN, Infinity, 1.5, "10"]) {
    assert.equal(
      locationBatchSchema.safeParse({ ...batchInput, warehouseId, count })
        .success,
      false,
    );
  }
  ...
  assert.equal(
    locationSchema.safeParse({ ...parts, warehouseId: { $ne: null } }).success,
    false,
  );
  assert.equal(
    locationSchema.safeParse({ ...parts, warehouseId, zone: "<script>" })
      .success,
    false,
  );
  ...
  for (const path of [
    ...
    assert.equal(locationReturnPath(path), undefined);
